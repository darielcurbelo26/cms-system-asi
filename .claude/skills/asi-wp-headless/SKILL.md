...
### Incidente: `wp rewrite flush --hard` no genera el `.htaccess` correcto cuando WP vive en una subcarpeta

Al migrar a producción real (hosting Hostinger, WordPress instalado en `public_html/cms/`), el endpoint `/cms/wp-json/tatc/v1/content` devolvía 404 aunque `siteurl`/`home` estaban correctos (`https://throughalltheclutter.com/cms`) y el fallback sin pretty-permalinks (`/cms/index.php?rest_route=/tatc/v1/content`) funcionaba bien — confirmando que era un problema de reglas de rewrite, no de datos. `wp option get permalink_structure` mostraba `/%postname%/` (permalinks bonitos sí activados). El diagnóstico: `wp rewrite flush --hard` reescribió el `.htaccess` con `RewriteBase /` y `RewriteRule . /index.php [L]` (como si WP viviera en la raíz), pero `wp eval 'global $wp_rewrite; echo $wp_rewrite->mod_rewrite_rules();'` mostraba las reglas correctas (`RewriteBase /cms/`, `RewriteRule . /cms/index.php [L]`) — es decir, WordPress **calcula bien** la ruta pero el comando de WP-CLI que escribe el archivo (`save_mod_rewrite_rules()`) tiene un bug/lógica distinta para instalaciones en subcarpeta. Fix aplicado: editar el `.htaccess` a mano (o con `sed`) para que el bloque `# BEGIN WordPress` use `RewriteBase /cms/` y `RewriteRule . /cms/index.php [L]`. Lección para cualquier migración futura a una subcarpeta: **no confiar en `wp rewrite flush --hard` para fijar el RewriteBase — verificar el `.htaccess` a mano después**, y comparar contra `$wp_rewrite->mod_rewrite_rules()` si algo no carga.
...
6. **Qué páginas están protegidas — corregido el 2026-06-25, ya no es una lista estática.** Originalmente `security.pages` solo vivía en `content.json` (un mapa fijo), totalmente desconectado de los `tatc_gate`: crear un gate en wp-admin guardaba el password pero **no protegía nada**, porque `script.js` nunca consultaba WordPress para saber qué páginas son privadas (bug real encontrado en producción — un gate creado para `blog.html` no bloqueaba nada). Fix: `tatc_get_custom_content()` ahora construye `security.pages` dinámicamente a partir de los `tatc_gate` existentes (cada uno marca su `page_file` como `"private"`), y `script.js`'s `checkSecurity()` pide esa lista a WordPress en paralelo con `content.json` (con timeout corto de ~2.5s para no demorar el redirect si WP tarda), combinando ambas — WordPress gana si hay conflicto. `content.json` sigue funcionando como respaldo si WordPress no responde a tiempo, pero crear un gate nuevo en wp-admin ahora **sí** lo protege de inmediato, sin tocar código.
7. **Trampa real encontrada al usar esto:** si alguien pega la URL completa (`https://dominio.com/blog.html`) en el campo `page_file` en vez de solo el nombre del archivo (`blog.html`), no hace match con el `lookupPage` que calcula `script.js` y el gate queda inerte. `functions.php` ahora normaliza esto automáticamente (`basename(wp_parse_url(...))`), pero si un gate parece no estar funcionando, revisar primero el valor exacto de `page_file` en esa entrada.
...
- [x] **En mobile, arrastrar el cubo 3D de `index.html` también hacía scroll de la página — corregido el 2026-06-22.** Causa: en `script.js`, el listener de `touchmove` del cubo estaba registrado con `{ passive: true }`, lo que le dice al navegador que el handler nunca llamará `preventDefault()` — el navegador hacía scroll/pull-to-refresh de inmediato sin esperar al JS. Fix: el listener pasó a `{ passive: false }` y ahora llama `e.preventDefault()` mientras `isDrag` es `true` (antes de mover el cubo). El CSS de `#cube-container` ya tenía `touch-action: none;` desde antes (no hizo falta tocarlo), pero por sí solo no bastaba sin el `preventDefault()` en JS. Verificado con Chrome headless + CDP simulando un touch-drag real: `window.scrollY`/`scrollTop` se mantienen en `0` durante todo el gesto, y el cubo sí se mueve según el drag.
- [x] **Migración real a producción completada — 2026-06-25.** Dominio real: `throughalltheclutter.com` (Hostinger, plan Premium). WordPress vive en `public_html/cms/`, frontend en la raíz de `public_html`. DNS, SSL y los 4 escenarios de "Verificación" probados contra el dominio real (no solo local). Acceso SSH configurado (`~/.ssh/hostinger_throughalltheclutter`, host `5.183.10.122:65002`, usuario `u488321774`, WP-CLI disponible) — permite hacer cambios reales en producción directo por terminal sin pasar por capturas del panel web (hPanel sigue siendo necesario solo para lo que vive exclusivamente ahí, como DNS o instaladores).
- [x] **Page Gate creado en wp-admin no protegía nada — corregido el 2026-06-25 (bug real encontrado en producción).** Ver la sección "Page Gate" arriba (puntos 6 y 7) para el detalle completo: `security.pages` ahora se construye dinámicamente desde los `tatc_gate`, con normalización de `page_file` por si alguien pega una URL completa en vez del nombre del archivo.
- [x] **Cache-busting agregado a `cms-engine.js` y `script.js` — 2026-06-25.** Ambos se cargaban con `<script src="archivo.js">` sin parámetro de versión, así que un navegador que ya hubiera visitado el sitio antes (con una versión vieja cacheada) podía seguir ejecutando código obsoleto después de un despliegue (causó confusión real: alguien viendo el sitio en producción con una copia vieja de `cms-engine.js` que aún apuntaba a la URL local de desarrollo). Convención adoptada: cada archivo lleva `?v=N` en su `<script src>`, y **hay que subir ese número manualmen­te en cada HTML que lo carga cuando se edite ese JS de forma significativa** (no hay build tool que lo automatice). Estado actual: ambos en `?v=2`.
...
- [x] `blog` + `post` → **Entradas nativas de WordPress** (2026-06-19). `tatc_build_blog_and_posts()` en `functions.php` arma `blog.posts[]` y `post.posts{}` desde `get_posts(post_type=post)`: `slug`=`post_name`, `date` en formato `d.m.y`, `subject`=nombre de la primera categoría asignada, `summary`=excerpt manual o auto-generado, `body[]` parseado de los bloques de Gutenberg (`core/paragraph`→`paragraph`, `core/image`→`image`, `core/quote`/`core/pullquote`→`pullquote`; otros tipos de bloque se ignoran por ahora). Verificado contra la entrada "Hello World" de ejemplo que WordPress trae por defecto — forma exacta. Cómo publicar: wp-admin → Posts → Add New → escribir con bloques Paragraph/Image/Quote → asignar una Categoría (= `subject`) → opcional: Excerpt manual en el panel de Document → Publish. El **slug** (permalink) de la entrada es el que se usa en `post.html?slug=...`, igual que antes.
